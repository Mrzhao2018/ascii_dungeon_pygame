...
class FloorManager:
    ...
    def process_floor_transition(
        self,
    ) -> Tuple[Optional[List[str]], Optional[entities.EntityManager], Optional[dict], Optional[Tuple[int, int]]]:
        ...
        try:
            ...
            self.game_state.write_exit_log(
                f'Generating floor {floor_number} with seed {gen_seed}, size {gen_width}x{gen_height}'
            )
