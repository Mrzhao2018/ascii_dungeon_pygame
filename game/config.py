...
class GameConfig:
    ...
    def parse_arguments(self):
        ...
        self.corridor_radius = self.parse_int_arg('--corridor-radius', None) or self._get_config_value(
            'game.corridor_radius', 1
        )

        # Camera parameters
        self.cam_lerp = self.parse_float_arg('--cam-lerp', None) or self._get_config_value('camera.lerp', 0.2)
        self.cam_deadzone = self.parse_float_arg('--cam-deadzone', None) or self._get_config_value(
            'camera.deadzone', 0.0
        )

        # Player parameters
        self.sprint_multiplier = self.parse_float_arg('--sprint-multiplier', None) or self._get_config_value(
            'player.sprint_multiplier', 0.6
        )
        self.sprint_cost = self.parse_float_arg('--sprint-cost', None) or self._get_config_value(
            'player.sprint_cost', 35.0
        )
        self.stamina_max = self.parse_float_arg('--stamina-max', None) or self._get_config_value(
            'player.stamina_max', 100.0
        )
        self.stamina_regen = self.parse_float_arg('--stamina-regen', None) or self._get_config_value(
            'player.stamina_regen', 12.0
        )
        self.sprint_cooldown_ms = self.parse_int_arg('--sprint-cooldown-ms', None) or self._get_config_value(
            'player.sprint_cooldown_ms', 800
        )

        # FOV parameters
        self.sight_radius = self.parse_int_arg('--sight-radius', None) or self._get_config_value('fov.sight_radius', 6)
        self.enable_fov = (
            '--enable-fov' in sys.argv
            if '--enable-fov' in sys.argv or '--disable-fov' in sys.argv
            else self._get_config_value('fov.enabled', True)
        )
        if '--disable-fov' in sys.argv:
            self.enable_fov = False

        # Logging configuration
        self.max_log_size = self.parse_int_arg('--max-log-size', None) or self._get_config_value(
            'logging.max_log_size', 512 * 1024
        )  # 512KB
        self.max_log_files = self.parse_int_arg('--max-log-files', None) or self._get_config_value(
            'logging.max_log_files', 3
        )
        self.enable_performance_logging = (
            '--enable-perf-logging' in sys.argv
            if '--enable-perf-logging' in sys.argv or '--disable-perf-logging' in sys.argv
            else self._get_config_value('logging.enable_performance_logging', False)
        )
        if '--disable-perf-logging' in sys.argv:
            self.enable_performance_logging = False
        self.log_only_important = '--verbose-logging' not in sys.argv and self._get_config_value(
            'logging.log_only_important', True
        )

        # Debug file configuration
        self.save_debug_levels = '--save-debug-levels' in sys.argv or self._get_config_value(
            'debug.save_debug_levels', False
        )
        self.max_debug_levels = self.parse_int_arg('--max-debug-levels', None) or self._get_config_value(
